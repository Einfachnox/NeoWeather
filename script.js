...
class NeoWeather {
  ...
  bindEvents() {
    ...
    document.addEventListener("click", (e) => {
      if (
        !this.themeMenuToggle.contains(e.target) &&
  ...
  initializeDesignThemes() {
    ...
    const savedDesignTheme =
      localStorage.getItem("designTheme") || "neobrutalism";
  ...
  async getCoordinates(city) {
    const url = `${this.geocodingUrl}?name=${encodeURIComponent(
      city
    ...
    return {
      ...
      country:
        data.results[0].country_code?.toUpperCase() || data.results[0].country,
    };
  }
  ...
  async fetchWeatherData(coordinates) {
    ...
    try {
      ...
      if (!currentResponse.ok) {
        ...
        throw new Error(
          `Current weather API error! status: ${currentResponse.status}`
        );
      }
      ...
      try {
        ...
        if (forecastResponse.ok) {
          ...
          console.warn(
            "Forecast data not available, using current weather only"
          );
          return currentData;
        }
      } catch (forecastError) {
        console.warn(
          "Forecast fetch failed, using current weather only:",
          forecastError
        );
        return currentData;
      }
  ...
  displayWeatherData(data, coordinates) {
    ...
    this.windSpeed.textContent = `${Math.round(
      data.current.wind_speed_10m
    )} KM/H`;
    this.pressure.textContent = `${Math.round(
    ...
    for (let i = 0; i < maxDays; i++) {
      ...
      const dayName =
        i === 0
          ...
          : i === 1
          ? "TOMORROW"
          : date.toLocaleDateString("en-EN", { weekday: "long" }).toUpperCase();
